...
python3 -m compileall zmatrix/research_db/account_truth/ tests/research_db/account_truth/ 2>&1 | tail -1

echo "[1] Phase 0 baseline..."
bash scripts/verify_research_db_phase0.sh > /dev/null 2>&1 && echo "  ✅ Phase 0" || { echo "  ❌ Phase 0"; exit 1; }

echo "[2] Private data guardrail..."
PYTHONPATH=. python3 -m pytest -q tests/research_db/account_truth/test_private_data_guardrail.py 2>&1 | tail -2
...
echo "[7] Full pytest..."
PYTHONPATH=. python3 -m pytest -q tests/research_db/account_truth/ 2>&1 | tail -3
...
test -f docs/research_db/PHASE1_ACCEPTANCE_MATRIX.md && echo "  ✅"
test -f docs/research_db/PHASE1_CLOSEOUT_REPORT.md && echo "  ✅"
