"""Executable formula for F56 MONEY_FLOW_STRENGTH.

Factor type: MONEY_FLOW_MICRO
Formula: sum(CLV*volume_5d)/sum(volume_5d)
...
def compute_signal(input_prices, rebalance_date):
    """Compute F56 MONEY_FLOW_STRENGTH signal scores.
    ...
    for ticker, bars in input_prices.items():
        ...
        for b in bars[-5:]:
            rng = b["high"] - b["low"]
            clv = (b["close"] - b["low"]) / max(rng, 0.0001)
            total_mf += clv * b["volume"]
            total_vol += b["volume"]
        scores[ticker] = round(total_mf / max(total_vol, 1), 6)
    return scores
