...
def validate_invalidation_anatomy_report(record: dict) -> list[str]:
    errors = []
    ...
    candidates = record.get("conditional_invalidation_candidates", {}).get("candidates", [])
    for c in candidates:
        if c.get("lookahead_risk") is True: errors.append(f"candidate {c.get('candidate_name')} has lookahead risk")
        if c.get("production_ready") is True: errors.append(f"candidate {c.get('candidate_name')} must not be production_ready")
