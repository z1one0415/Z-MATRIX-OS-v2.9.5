#!/usr/bin/env bash
set -euo pipefail
echo "═══ Z-MATRIX-OS v3.3 Real BRD Connection Verification ═══"
echo ""; echo "== Compile =="; PYTHONPATH=. python3 -m compileall zmatrix tests scripts pipelines
echo "✅ compileall PASS"
for t in test_real_brd_connector test_brd_classifier_connection test_brd_connection_validator test_brd_strategy_connected_report \
  test_brd_pit_feature_builder test_brd_classifier_adapter test_brd_paper_action_builder \
...
echo "✅ clean"
echo ""; echo "═══ v3.3 Verification PASS ═══"
