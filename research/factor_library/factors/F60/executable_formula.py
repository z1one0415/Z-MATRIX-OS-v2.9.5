"""Executable formula for F60 INTRADAY_REVERSAL_PRESSURE.

Factor type: MONEY_FLOW_MICRO
Formula: -mean((close-open)/(high-low), 3d)
...
def compute_signal(input_prices, rebalance_date):
    """Compute F60 INTRADAY_REVERSAL_PRESSURE signal scores.
    ...
    for ticker, bars in input_prices.items():
        if len(bars) < 3: scores[ticker]=0.0; continue
        ps=[]
        for b in bars[-3:]:
            rng=b["high"]-b["low"]
            ps.append(-(b["close"]-b["open"])/max(rng,0.0001))
        scores[ticker]=round(sum(ps)/len(ps),6)
    return scores
