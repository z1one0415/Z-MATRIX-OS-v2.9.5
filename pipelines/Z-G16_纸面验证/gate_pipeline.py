...
PAPER_PROBE仅限conditional_output, 不作为默认action
"""
...
def run(proposal=None):
    ...
    full_ok = (dq_total >= 85 and mt_status == "PASS" and l1_5_safe 
               and world_ok and role_valid and tail_ok and hibernation_ok)
    
    if not full_ok:
        mode = "Lite"
        output_level = "O2_DIAGNOSTIC" if mt_status == "BLOCK" else "O3_CONDITIONAL"
        reasons = []
        if dq_total < 85: reasons.append(f"DQ={dq_total}<85")
        if mt_status != "PASS": reasons.append(f"MT={mt_status}")
        if not l1_5_safe: reasons.append("L1.5!=SAFE")
        if not world_ok: reasons.append("world!=PAPER_WORLD")
        if not role_valid: reasons.append("role_invalid")
        if not tail_ok: reasons.append("TailRisk=DEFENSIVE")
        if not hibernation_ok: reasons.append("Hibernation=NO_ATTACK")
        print(f"⚠️ Lite模式: {', '.join(reasons)}")
        result.update({
            "mode":mode, "output_level":output_level,
            "forbidden_fields":["price_zones","position_playbook","PAPER_PROBE","fill_price"],
            "route_hypotheses":[
                {"id":"A","condition":"L3→FULL+MT恢复PASS","action":"WAIT"},
                {"id":"B","condition":"缩量横盘","action":"WAIT"},
                {"id":"C","condition":"板块退潮","action":"WATCH"}
            ],
            "evidence_gaps":[f"DQ={dq_total}" if dq_total<85 else f"MT={mt_status}"],
            "waiting_triggers":["DQ恢复≥85","MT恢复PASS","价格交叉验证通过","L1.5 SAFE","world=PAPER_WORLD"]
        })
        return result
    
    # Full mode
    mode = "Full"
    print(f"Full模式: DQ={dq_total}, MT=PASS, L1.5=SAFE, world=PAPER_WORLD, role_valid, TailRisk/Hibernation cleared")
    result["mode"] = mode
    ...
    result["capability_mask"] = {
        ...
        "conditional_outputs": [{
            "output": "PAPER_PROBE",
            "requires": ["world=PAPER_WORLD","MT=PASS","L1.5=SAFE","role_valid=true","TailRisk!=DEFENSIVE_ONLY","Hibernation!=NO_ATTACK"]
        }],
    ...
    prob_req = ["world=PAPER_WORLD","MT=PASS","L1.5=SAFE","role_valid=true","TailRisk!=DEFENSIVE","Hibernation!=NO_ATTACK"]
    
    result["coach_plan"] = {
        "current_state": {"l3":"PARTIAL","dq":dq_total,"price":price},
        "scenario_routes": [
            {"id":"A顺风","condition":"L3→FULL","weight":"中","conditional_output":"PAPER_PROBE","paper_size":"1U","requires":prob_req},
            {"id":"B震荡","condition":"缩量横盘","weight":"中高","action":"WAIT"}
        ],
        ...
        "position_playbook": {"max":"3U","steps":[
            ...
            {"stage_id":"PROBE_1","conditional_output":"PAPER_PROBE","paper_size":"1U","requires":prob_req},
            {"stage_id":"PROBE_2","conditional_output":"PAPER_PROBE","paper_size":"1U","requires":prob_req}
        ]},
        "kill_conditions": ["MT_BLOCK","L3_REJECTED","放量跌破失效位"],
        "rollback_conditions": [{"from":"PROBE","to":"TRACK"},{"from":"A","to":"B"}]
    }
    
    print(f"\n📋 [1/8] 当前状态: role={proposal['selected_role']}, DQ={dq_total}, price={price}")
    print(f"📊 [2/8] 路线: A顺风(cond:PAPER_PROBE)/B震荡(WAIT) (uncalibrated)")
    print(f"💰 [3/8] 价格: watch={ma20*0.98:.1f}-{ma20*1.02:.1f}")
    print(f"📐 [4/8] 仓位: 3U制 (PROBE仅conditional_output)")
    print(f"🚦 [5/8] 信号: advance(L3→FULL+VWAP) risk_reduce(跌破失效+RETREAT)")
    print(f"⏰ [6/8] 检查点: T-1→0925→0935→0945→1030→1430→1500")
    print(f"🛑 [7/8] Kill:6条 Rollback:4条")
    print(f"📝 [8/8] 后验: T+1/3/5/20→Z9→Alpha验证仓")
    print(f"🏁 paper_only | PAPER_PROBE=conditional | 不连接券商")
