# Z9 Review Node Implementation Planning — Merge Risk Register
## Status: Z_SKILLOS_Z9_REVIEW_NODE_IMPLEMENTATION_PLANNING_SEALED

## 1. Status
Document type: Merge risk register. Part of Z9 Review Node Implementation Planning package.
...
Defines future merge risk register for Z9 Review Node disabled-default P0.
...
|:--:|:--|:--:|:--:|:--|:--|:--|
| 1 | Planning docs merged without depth verification | HIGH | LOW | Line count gate | CI check | Under minimum |
| 2 | Planning docs merged without key content | HIGH | LOW | Content grep gate | CI check | Missing key phrase |
| 3 | Non-docs introduced at merge | CRITICAL | LOW | Whitelist filter | Diff check | Non-doc file |
| 4 | Research files leak into postmerge | CRITICAL | LOW | Blacklist filter | Diff check | Research file |
| 5 | Z9 implemented without human approval | CRITICAL | LOW | Approval gate docs | Branch gate | Pre-human code |
| 6 | Z9 implementation conflicts with planning | MEDIUM | LOW | Planning doc holds contract | Diff against plan | Implementation mismatch |
| 7 | Z2 snapshot contract incomplete at impl | MEDIUM | MEDIUM | Snapshot schema in planning | Review at impl | Missing fields |
| 8 | Evidence chain undefined at impl | HIGH | LOW | Evidence plan in planning | Evidence test | Missing hash |
| 9 | Degradation policy undefined at impl | HIGH | LOW | Degradation plan in planning | Degradation test | No degrade path |
| 10 | Attribution leaks profit at impl | CRITICAL | LOW | Attribution policy | Attribution test | Profit attribution |
| 11 | Z9 feedback mutates at impl | CRITICAL | LOW | Feedback policy | Feedback test | Auto-mutation |
| 12 | Memory mutation introduced at impl | CRITICAL | LOW | Degradation blocks | Mutation test | Memory write |
| 13 | Trade/broker linkage at impl | CRITICAL | LOW | Blocked output policy | Import scan | Broker import |
| 14 | Planning docs stale after long delay | MEDIUM | MEDIUM | Version check before merge | Base HEAD verify | Base drift |
| 15 | Reviewer misses depth requirement | MEDIUM | MEDIUM | Depth verification script | CI pre-merge | Under minimum |
| 16 | Implementation branch diverges from planning | MEDIUM | LOW | Planning ref in impl gate | Diff check | Scope drift |
| 17 | Merge without full test suite | HIGH | MEDIUM | Test gate in merge checklist | CI block | Tests not run |
| 18 | Level 5 boundary breached at impl | CRITICAL | LOW | Level 5 BLOCKED in planning | Doc audit | Level 5 language |
| 19 | Multiple Z9 impl branches conflict | MEDIUM | LOW | Single branch policy | Branch list | Multiple impl |
| 20 | Planning seal missing at merge | HIGH | LOW | Seal file required | Merge gate | No seal file |