"""Enhanced formula for F12 VALUE_COMPOSITE.
Primitives: pb_inv, pe_inv
Enhanced with pb_inv and pe_inv proxies — value detection from price decline
...
def compute_signal(input_prices, rebalance_date):
    """Compute F12 VALUE_COMPOSITE (enhanced variant)."""
    scores = {}
    # F12 requires daily_basic data (PE/PB). Without it, fall back to price-based value proxy.
    for ticker, bars in input_prices.items():
        if len(bars) < 5:
            scores[ticker] = 0.0; continue
        # No PE/PB from price_bars alone. Use price-based value proxy: low P/B approximated by price decline
        ret_60d=(bars[-1]["close"]-bars[-min(60,len(bars))]["close"])/max(bars[-min(60,len(bars))]["close"],0.0001)
        # Proxy: stocks that have declined more have higher value score (value = mean reversion)
        scores[ticker]=round(-ret_60d,6)
    return scores
