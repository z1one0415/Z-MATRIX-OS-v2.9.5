...
blocked="BLOCKED"in g.get("status","");reasons=g.get("blocking_reasons",[])
ready_ok=g["ready_for_alpha_operating_loop"]is False if blocked else True
research_ok=g["research_only"]is False if blocked else True
reasons_ok=len(reasons)>0 if blocked else len(reasons)==0
