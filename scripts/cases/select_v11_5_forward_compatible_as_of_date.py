#!/usr/bin/env python3
"""Select forward-compatible as_of_date by scanning dates with snapshot ticker check."""
import json
...
best_date=None;best_cov=-1;best_e=0;best_f=0
for ad in all_dates:
    if ad not in fv_by:continue
    bt=set()
    for wi in eligible:
        fid=wi["factor_id"];rd=wi["rankic_direction"];vals=[]
        for tk,factors in fv_by[ad].items():
            if fid in factors:vals.append((tk,factors[fid]["value"]))
        vals.sort(key=lambda x:x[1]);n=len(vals)
        if n<10:continue
        if rd=="NEGATIVE":bt.update(t for t,v in vals[:max(1,n//5)])
        elif rd=="POSITIVE":bt.update(t for t,v in vals[-max(1,n//5):])
    e=0;f=0
    for hn in["T20","T60"]:
        for tk in bt:e+=1
        if(ad,tk,hn)in lab_by:f+=1
    cov=f/e if e>0 else 0
    if cov>=best_cov and e>0:best_date,best_cov,best_e,best_f=ad,cov,e,f
mode="FULL"if best_cov==1.0 else"PARTIAL"
d={"status":f"V11_5_FORWARD_COMPATIBLE_AS_OF_DATE_SELECTED_{mode}","selected_as_of_date":best_date,"coverage_mode":mode,"expected_label_count":best_e,"found_label_count":best_f,"missing_label_count":best_e-best_f,"coverage":round(best_cov,6),"latest_full_coverage_selected":best_cov==1.0,"ready_for_full_portfolio_simulation":best_cov==1.0,"ready_for_partial_portfolio_report":True}
json.dump(d,open(C/"v11_5_forward_compatible_as_of_date.json","w"),indent=2)
print(f"Selector: {best_date} cov={best_cov:.6f} {mode} e={best_e} f={best_f}")
