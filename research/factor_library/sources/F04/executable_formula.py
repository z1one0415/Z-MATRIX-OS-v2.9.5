"""Executable formula for F04 RESIDUAL_MOMENTUM.

Factor type: PRICE_VOLUME_RISK
Formula: cumulative_return(t-20, t-5)
...
def compute_signal(input_prices, rebalance_date):
    """Compute F04 RESIDUAL_MOMENTUM signal scores.
    ...
    for ticker, bars in input_prices.items():
        if len(bars) < 5:
            scores[ticker] = 0.0
            continue
        return_20d = (bars[-5]["close"] - bars[-20]["close"]) / max(bars[-20]["close"], 0.0001)
        scores[ticker] = round(return_20d, 6)
    return scores
