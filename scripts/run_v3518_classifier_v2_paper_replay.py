...
def main():
    ...
    for r in rows:
        role,src=_classify_v2(r)
        # P0-4 FIX: real trace readiness check
        raw = r.get("source_brd_result",{}).get("source_raw",{}) or {}
        is_ready = bool(raw) and (any(isinstance(raw.get(k),dict) and len(raw.get(k,{}))>0 for k in ("b_matrix","r_matrix","d_matrix")) or any(k in raw for k in ("b_matrix_pass","r_matrix_pass","d_matrix_pass")))
        if is_ready: trace_ready+=1
        r["classifier_v2_trace_ready"]=is_ready
