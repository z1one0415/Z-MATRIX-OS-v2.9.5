#!/usr/bin/env python3
"""V12 Gate — reads ALL closeouts, strict fail-closed, research_only=not blocked."""
import json
...
def main():
    ...
    v6_trk = v6.get("paper_tracking_period_completed", False)
    v7 = load("v11_7_closeout.json")
    v7_ok = v7.get("status") == "V11_7_COST_MODEL_UPGRADE_CONFIRMED"
    v7_cst = v7.get("cost_model_status", "PROXY_ONLY") != "PROXY_ONLY"
    ...
    v9_atp = v9.get("autopsy_input_ready", False)
    reasons = []
    if not v5_ok: reasons.append("V11_5_NOT_CONFIRMED")
    if not v6_ok: reasons.append("V11_6_NOT_CONFIRMED")
    if not v6_trk: reasons.append("PAPER_TRACKING_PERIOD_NOT_COMPLETED")
    if not v7_ok: reasons.append("V11_7_NOT_CONFIRMED")
    if not v7_cst: reasons.append("COST_MODEL_PROXY_ONLY")
    ...
    if not v9_atp: reasons.append("Z9_AUTOPSY_INPUT_NOT_READY")
    ...
    print(f"V12 Gate: {g['status']} | research_only={g['research_only']} | reasons={reasons}")
