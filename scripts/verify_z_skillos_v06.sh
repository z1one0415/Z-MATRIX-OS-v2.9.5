#!/usr/bin/env bash
set -euo pipefail
echo "═══ Z-SkillOS v0.6.1 Factor Hardened ═══"
...
echo "═══ v0.6.1 Factor All-Skills Runtime ═══"
PYTHONPATH=. python3 -c "
...
echo "═══ v0.6.1 Forbidden Scan ═══"
PYTHONPATH=. python3 -c "
from pathlib import Path
pairs=[('external_api_used','True'),('shadowbroker_deployed','True'),('production_allowed','True'),('trade_allowed','True'),('verdict_allowed','True'),('broker_order_allowed','True'),('real_trade_allowed','True'),('auto_buy_allowed','True'),('auto_sell_allowed','True'),('factor_calculation_allowed','True'),('backtest_allowed','True'),('ranking_allowed','True'),('trading_signal_allowed','True')]
...
for r in ['zmatrix','scripts','tests/agent','data/research_db/agent/registry']:
 ...
 for f in p.rglob('*'):
  ...
  for k,v in pairs:
   assert f'{k}={v}' not in t,f'{k}={v} in {f}'
