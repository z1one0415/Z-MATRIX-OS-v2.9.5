...
def test_order_validate_buy(): a=PaperAccount("A1"); oe=PaperOrderEngine(a); o=oe.create_order("O1","000001","BUY",100000,10); assert oe.validate_order(o) is False  # exceeds cash
def test_order_validate_sell_no_pos(): a=PaperAccount("A1"); oe=PaperOrderEngine(a); o=oe.create_order("O1","000001","SELL",100,10); assert oe.validate_order(o) is False
...
def test_overfit_detected(): r=WalkForwardRunner.run("E2",[{"ic":0.10}]*5,[{"ic":0.08}]*5,[{"ic":0.01}]*5); assert r.overfit_detected is True
...
def test_stress_2008(): r=StressLab.apply_market_stress(1e6,"2008_GFC"); assert r.drawdown<0; assert r.survived is True
