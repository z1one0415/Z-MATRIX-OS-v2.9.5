...
def validate_joint_attribution_report(record: dict) -> list[str]:
    errors = []
    ...
    for c in (record.get("joint_candidates",{}).get("candidates") or []):
        if c.get("lookahead_risk") is True: errors.append(f"{c.get('candidate_name')} has lookahead risk")
        if c.get("production_ready") is True: errors.append(f"{c.get('candidate_name')} must not be production_ready")
    s = record.get("safety",{})
