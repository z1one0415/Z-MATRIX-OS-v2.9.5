"""Enhanced formula for F10 LOW_VOLATILITY.
Primitives: vol_20d
Enhanced with vol_20d primitive — penalizes high volatility more aggressively
...
def compute_signal(input_prices, rebalance_date):
    """Compute F10 LOW_VOLATILITY (enhanced variant)."""
    scores = {}
    for ticker, bars in input_prices.items():
        if len(bars) < 20:
            scores[ticker] = 0.0; continue
        returns = [(bars[i]["close"]-bars[i-1]["close"])/max(bars[i-1]["close"],0.0001) for i in range(max(1,len(bars)-20),len(bars))]
        if not returns: scores[ticker]=0.0; continue
        m=sum(returns)/len(returns)
        std=(sum((r-m)**2 for r in returns)/len(returns))**0.5
        # Enhanced: vol_20d primitive — lower vol = higher score
        vol_20d = std
        scores[ticker] = round(1.0 / (1.0 + vol_20d * 100) * (1.0 - vol_20d * 10), 6)
    return scores
