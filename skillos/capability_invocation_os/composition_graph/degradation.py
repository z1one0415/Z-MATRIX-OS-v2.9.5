...
def build_disabled_default_response(response_id: str = "") -> CompositionGraphResponse:
    return CompositionGraphResponse(
        ...
        degraded=True, mode=GRAPH_MODE, graph_enabled=False, execution_enabled=False,
    )
...
def build_denied_source_response(response_id: str = "", reason: str = "") -> CompositionGraphResponse:
    return CompositionGraphResponse(
        ...
        nodes=[], edges=[], evidence={"reason": reason},
        forbidden_outputs_removed=sorted(FORBIDDEN_GRAPH_OUTPUTS),
        degraded=True, mode=GRAPH_MODE, graph_enabled=False, execution_enabled=False,
    )
...
def build_denied_dag_invalid_response(response_id: str = "") -> CompositionGraphResponse:
    return CompositionGraphResponse(
        response_id=response_id,
        decision=CompositionGraphDecision.DENY_GRAPH_DAG_INVALID,
        nodes=[], edges=[], evidence={},
        forbidden_outputs_removed=sorted(FORBIDDEN_GRAPH_OUTPUTS),
        degraded=True, mode=GRAPH_MODE, graph_enabled=False, execution_enabled=False,
    )
...
def build_allowed_readonly_response(
    ...
    return CompositionGraphResponse(
        ...
        forbidden_outputs_removed=sorted(FORBIDDEN_GRAPH_OUTPUTS),
        degraded=False, mode=GRAPH_MODE, graph_enabled=False, execution_enabled=False,
    )
