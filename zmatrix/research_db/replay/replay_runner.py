...
class ReplayRunner:
    ...
    def run_rolling_replay(self, experiment_id: str, start: str, end: str, window: int = 20, step: int = 5) -> ReplayResult:
        ...
        try:
            slices = ds.generate_rolling_slices(start, end, window, step)
        except ValueError as e:
            error_msg = str(e)
            error_type = "MALFORMED_TRADING_CALENDAR" if "MALFORMED" in error_msg else "CALENDAR_ERROR"
            result = ReplayResult(experiment_id=experiment_id, status="FAILED")
            result.details = {"error_type": error_type, "message": error_msg}
            return result
        return self.run_replay(experiment_id, slices)
