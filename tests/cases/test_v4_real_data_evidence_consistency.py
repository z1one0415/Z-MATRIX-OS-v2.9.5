...
@pytest.fixture
def rd(): return json.loads((W/"runtime_reports"/"cases"/"core_12_real_market_data_readiness.json").read_text())
@pytest.fixture
def rr(): return json.loads((W/"runtime_reports"/"cases"/"core_12_real_return_readiness.json").read_text())
@pytest.fixture
def gate(): return json.loads((W/"runtime_reports"/"cases"/"v5_entry_gate.json").read_text())
...
def test_adj_equals_non_missing(rd,rr):
    c=sum(1 for r in rr["cases"] if r["adjustment_factor_status"]!="MISSING")
    assert rd["summary"]["adjustment_ready"]==c
